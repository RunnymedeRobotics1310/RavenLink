...
- (void)show_menu
{
  // RavenLink patch: upstream positioned the menu at
  //   NSMakePoint(0, button.bounds.size.height + 6)
  // which, in the NSStatusBarButton's (non-flipped) coordinate space,
  // lands *above* the top of the screen. macOS then clamps the menu
  // and marks it as scrolled-up, showing a ^ arrow that hides the
  // first item(s) until the user scrolls.
  //
  // Position the menu's top-left at the bottom-left of the button so
  // it flows downward from below the menu bar, with no scroll flag.
